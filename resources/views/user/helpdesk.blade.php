    ...
    <div class="space-y-4">
        <template x-for="ticket in filteredTickets" :key="ticket.id">
            <div class="bg-white rounded-2xl sm:rounded-3xl border border-[#eff3f4] p-4 sm:p-6 hover:border-[#bde2f9] transition-all shadow-2xs">
                ...
                <!-- Thread Replies -->
                <div class="mt-4 pt-4 border-t border-[#eff3f4] space-y-3" x-show="ticket.replies && ticket.replies.length > 0" x-cloak>
                ...
                <!-- Fast Reply Form -->
                <div class="mt-3 pt-3 border-t border-[#eff3f4] flex gap-2" x-show="ticket.status === 'open' && $store.app.activeStoreEventActive" x-cloak>
                    <input 
                        type="text" 
