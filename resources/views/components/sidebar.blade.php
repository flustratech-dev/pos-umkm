            ...
            <div class="space-y-1">
                ...
                >
                    ...
                    <template x-if="$store.app?.stats?.pendingCount > 0">
                        <span class="px-2 py-0.5 text-[10px] font-black rounded-full bg-[#ff7a00] text-white" x-text="$store.app.stats.pendingCount"></span>
                    </template>
                </a>

                <a 
                    href="{{ route('admin.verifikasi-cash.index') }}" 
                    class="flex items-center justify-between px-4 py-2.5 rounded-full text-sm font-medium transition-all group cursor-pointer {{ request()->is('admin/verifikasi-cash*') ? 'bg-[#e8f5fd] text-[#1d9bf0] font-bold' : 'text-[#0f1419] hover:bg-[#eff3f4]' }}"
                >
                    <div class="flex items-center gap-3">
